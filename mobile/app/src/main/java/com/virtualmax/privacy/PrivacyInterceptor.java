...
public class PrivacyInterceptor {
    ...
    /**
     * Движок пользовательских меток (галочек/бейджей).
     * Идентичен desktop-версии (desktop/src/preload/badges.js).
     * Плейсхолдер __VM_LIST__ заменяется на JSON-массив меток.
     */
    private static final String BADGES_ENGINE =
        "(function(){'use strict';" +
    ...
    /** Собирает JS-инъекцию движка меток для готового JSON-массива. */
    public static String buildBadgesJs(String listJson) {
        String list = (listJson == null || listJson.trim().isEmpty()) ? "[]" : listJson;
        return BADGES_ENGINE.replace("__VM_LIST__", list);
    }
