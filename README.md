...
5. **📎 Отправка файлов**:
   - Выбор фото, видео и документов из галереи/файлового менеджера, съёмка с камеры прямо из мессенджера (Android).

6. **📥 Менеджер загрузок**:
   - Сохранение медиа в папку «Загрузки» с системным разрешением на старых Android.

7. **🧹 Экстренная очистка**:
   - Удаление cookies, LocalStorage и кэша в один клик.

8. **⚡ Оптимизация для слабых телефонов**:
   - Лёгкий сетевой фильтр без regex на каждый запрос, пакетное обновление счётчика блокировок, дорендеринг соседних экранов, остановка рендерера в фоне, восстановление после его падения.

9. **🎛️ Удобство интерфейса**:
   - Кнопки «Назад/Вперёд», индикатор загрузки, главный переключатель блокировки трекеров, возврат скрытых панелей плавающей кнопкой, адаптивная иконка.
